  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        ...
        const scheduled = scheduledRes.ok ? (await scheduledRes.json()).length : 0;
        const published = publishedRes.ok ? (await publishedRes.json()).length : 0;
  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] overflow-hidden">
      ...
      <aside className={cn(
        ...
        <div className={cn("p-4 border-b border-gray-200 dark:border-gray-800", collapsed && "px-2")}>
          {collapsed ? (
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  isActive ? "bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-300" : "text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800",
                ...
                {!collapsed && (
                  <>
                    ...
                    {item.badge !== undefined && item.badge > 0 && (
                      <span className="bg-gray-200 dark:bg-gray-700 px-1.5 text-xs rounded-full">{item.badge}</span>
                    )}
                  </>
                )}
              </button>
        ...
        {!collapsed && (
          <div className="border-t border-gray-200 dark:border-gray-800 p-4">
            <h4 className="text-xs font-medium text-gray-500 uppercase mb-3">This Week</h4>
            <div className="grid grid-cols-2 gap-3">
              <div className="rounded-lg bg-green-50 dark:bg-green-950 p-3 text-center">
                <p className="text-lg font-bold text-green-700 dark:text-green-400">{stats.published}</p>
                <p className="text-xs text-green-600">Published</p>
              </div>
              <div className="rounded-lg bg-amber-50 dark:bg-amber-950 p-3 text-center">
                <p className="text-lg font-bold text-amber-700 dark:text-amber-400">{stats.scheduled}</p>
...
function DraftsView({ companyId, onEdit }: { companyId: string; onEdit?: () => void }) {
  ...
  useEffect(() => {
    ...
    const fetchDrafts = async () => {
      setLoading(true);
      try {
        const response = await fetch(`/api/posts?companyId=${companyId}&status=DRAFT`);
        if (response.ok) {
                ...
                <div className="flex-1">
                  ...
                  {draft.hashtags && draft.hashtags.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      {draft.hashtags.slice(0, 5).map((tag: string, index: number) => (
                        <span key={index} className="text-blue-600 dark:text-blue-400 text-xs">
                          #{tag}
                        </span>
                      ))}
                      {draft.hashtags.length > 5 && (
                        <span className="text-gray-500 text-xs">+{draft.hashtags.length - 5} more</span>
                      )}
                    </div>
                  )}
                </div>
...
function QueueView({ companyId }: { companyId: string }) {
  ...
  useEffect(() => {
    ...
    const fetchScheduled = async () => {
      setLoading(true);
      try {
        const response = await fetch(`/api/posts?companyId=${companyId}&status=SCHEDULED`);
        if (response.ok) {
          const data = await response.json();
          setScheduled(data);
        }
            ...
            <div key={date}>
              <h2 className="text-sm font-medium text-gray-500 mb-3 flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 rounded-full text-xs">
                  {groupedByDate[date].length} post{groupedByDate[date].length > 1 ? "s" : ""}
                </span>
              </h2>
                      ...
                      <div className="flex-1">
                        ...
                        {post.postMedia && post.postMedia.length > 0 && (
                          <div className="flex gap-1 mt-2">
                            {post.postMedia.slice(0, 3).map((pm: any, index: number) => (
                              <div key={index} className="w-8 h-8 rounded bg-gray-100 dark:bg-gray-800 overflow-hidden">
                                {pm.media?.mimeType?.startsWith("image/") ? (
                                  <img src={pm.media.url} alt="" className="w-full h-full object-cover" />
                                ) : (
